...
export function renderOrdersTable() {
    ...
    <tr>
      ...
      <td>
        <div class="action-btns">
          ...
          <button class="btn-visibility" data-idx="${i}" title="${o.isHidden ? 'Hidden from viewers. Click to show.' : 'Visible to viewers. Click to hide.'}">
            ${o.isHidden
  ...
  // Attach handlers
  tbody.querySelectorAll('.btn-visibility').forEach(btn => {
    btn.addEventListener('click', () => {
      const idx = parseInt(btn.dataset.idx);
      const order = store.orders[idx];
      if (!order) return;
      store.setOrderHidden(order.id, !order.isHidden);
    });
  });
